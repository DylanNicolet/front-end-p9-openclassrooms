...
export default class Login {
  ...
  handleSubmitAdmin = e => {
    const user = {
      type: "Admin",
      email: e.target.querySelector(`input[data-testid="admin-email-input"]`).value,//Bug report - Login: Changed input[employee] to input[admin] instead
      password: e.target.querySelector(`input[data-testid="admin-password-input"]`).value,//Bug report - Login: Changed input[employee] to input[admin] instead
      status: "connected"
    }
    this.localStorage.setItem("user", JSON.stringify(user))
    const userExists = this.checkIfUserExists(user)
